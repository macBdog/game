...
public:
	...
	// Neighbour accessors
	inline cLinkedListNode * GetNext() { return m_next; }
	inline cLinkedListNode * GetPrev() { return m_prev; }
...
private:
	...
	cLinkedListNode * m_next;
	cLinkedListNode * m_prev;

	// Allows list and iterators to access neighbours without function calls
	template <class T> friend class cLinkedList;
...
public:
	...
	inline cLinkedListNode<T> * GetHead() { return m_head; }
	inline cLinkedListNode<T> * GetTail() { return m_tail; }
	inline bool IsEmpty() { return m_length == 0; }

	inline void Insert(cLinkedListNode<T> * a_node)
	{
	...
	inline void Remove(cLinkedListNode<T> * a_node) 
	{
	...
	inline void Swap(cLinkedListNode<T> * a_node1, cLinkedListNode<T> * a_node2)
	{
		// Copy nodes to be swapped so we dont write over neighbour addresses
		// that are used later in the swap.
		cLinkedListNode<T> a_temp1;
		a_temp1 = *a_node1;

		cLinkedListNode<T> a_temp2;
		a_temp2 = *a_node2;
	...
	inline void Merge(cLinkedList<T> * a_result, cLinkedList<T> & a_list1, cLinkedList<T> & a_list2)
	{
		while (!a_list1.IsEmpty() && !a_list2.IsEmpty())
		{
			// Merge two lists in this list at the cursor preserving order
			cLinkedListNode<T> * cursor1 = a_list1.m_head;
			cLinkedListNode<T> * cursor2 = a_list2.m_head;
			cLinkedListNode<T> * insert = NULL;
		...
		{
			cLinkedListNode<T> * cursor = a_list1.m_head;
			a_list1.Remove(cursor);
			a_result->Insert(cursor);
		}
		while (!a_list2.IsEmpty())
		{
			cLinkedListNode<T> * cursor = a_list2.m_head;
...
private:
	...
	inline void MergeSort(cLinkedList<T> * a_list);

	// Pointers to head, tail and current operating position in the ist
	cLinkedListNode<T> * m_head;
	cLinkedListNode<T> * m_tail;
	cLinkedListNode<T> * m_cursor;
...
template <class T> void cLinkedList<T>::SlowSort() 
{
	...
	cLinkedListNode<T> * start = m_head;
...
template <class T> void cLinkedList<T>::MergeSort() 
{
	MergeSort(this);
}

template <class T> void cLinkedList<T>::MergeSort(cLinkedList<T> * a_list)
{
	// A list of size 0 or 1 is already sorted
