...
struct ParticleDefinition
{
    ...
    static constexpr std::string_view s_propertyNames[s_maxProperties] = {
        "lifeTime", "startPos", "startSize", "endSize",
        "startColour", "endColour", "startVel", "endVel"
    };
...
public:
    ...
    bool Startup(const Colour & a_clearColour, std::string_view a_shaderPath, const DataPack * a_dataPack, bool a_vr = false);
    bool Shutdown();
    ...
    void ManageShader(GameObject * a_gameObject, std::string_view a_shaderName);
    void ManageShader(Scene * a_scene, std::string_view a_shaderName);
    ...
    //\return true if the shader was compiled and allocated successfully
    static bool InitShaderFromMemory(const char * a_vertShaderSrc, const char * a_fragShaderSrc, Shader & a_shader_OUT);
...
private:
    ...
    Shader * GetShader(std::string_view a_shaderName);
    ...
    std::string m_shaderPath;										///< Path to the shader files
