...
public:
    ...
    ScriptManager(float a_updateFreq = s_updateFreq)
        : m_globalLua(nullptr), m_gameLua(nullptr), m_lastFrameDelta(0.0f), m_updateFreq(a_updateFreq), m_updateTimer(0.0f), m_forceReloadScripts(false)
    {
    }
    ~ScriptManager() { Shutdown(); }

    //\brief Set clear colour buffer and depth buffer setup
    bool Startup(std::string_view a_scriptPath, const DataPack *a_dataPack);
    bool Shutdown();
    ...
    inline void ReloadScripts()
    {
        m_updateTimer = m_updateFreq + 1.0f;
        m_forceReloadScripts = true;
    }
...
private:
    ...
    static constexpr std::string_view s_mainScriptName = "game.lua"; ///< Constant name of the main game script file
    ...
    {
        ManagedScript(std::string_view a_scriptPath, const FileManager::Timestamp &a_timeStamp)
            : m_path(a_scriptPath), m_timeStamp(a_timeStamp)
        {
        }
        FileManager::Timestamp m_timeStamp; ///< When the script file was last edited
        std::string m_path;					///< Where the script resides for reloading
    };
    ...
    std::string m_scriptPath;			///< Cache off path to scripts
