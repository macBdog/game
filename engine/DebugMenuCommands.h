...
enum class DebugMenuCommandAlign : int
{
...
struct DebugCommandReturnData
{
    ...
    DirtyFlag m_dirtyFlag;	///< If the command touched a widget or game object it should set a flag
    EditMode m_editMode;		///< If the command set a new mode for the debug menu
    EditType m_editType;		///< If the command set a new type of editing for the debug menu
...
public:
    ...
    DebugMenuCommand(const char * a_name, Widget * a_parent, Colour a_colour, EditType a_parentMenu);
    ~DebugMenuCommand() { }

    //\brief Accessors
    inline Widget * GetWidget() const { return m_widget; }
    inline DebugMenuCommandAlign GetAlignment() const { return m_alignment; }
    inline EditType GetParentMenu() const { return m_parentMenu; }
    ...
    void SetAlignment(Widget * a_alignedTo, DebugMenuCommandAlign a_alignment);
    void SetWidgetAlignment(DebugMenuCommandAlign a_alignment);
...
private:
    ...
    DebugMenuCommandAlign m_alignment;									///< How the command button is aligned to it's parent command NOTE: this is separate to widget alignment
    Widget * m_widget;															///< Visual representation of the command button
    EditType m_parentMenu;												///< Which menu the command should appear in, None means root
};
...
private:
    ...
    DebugMenuCommand * Create(const char * a_name, Widget * a_parent, Widget * a_alignTo, DebugMenuCommandAlign a_align, Colour a_colour, EditType a_parentMenu);
    ...
    const int s_numDebugMenuCommands[static_cast<int>(EditType::Count)] = {12, 10, 5}; ///< Number of commands in each menu, used for calculating menu size
