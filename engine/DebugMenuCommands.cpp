...
DebugCommandReturnData DebugMenuCommandRegistry::HandleRightClick(Widget * a_selectedWidget, GameObject * a_selectedGameObject, Light * a_selectedLight)
{
    ...
    // Figure out which debug menu to show
    auto menu_type = EditType::None;
    if (a_selectedWidget != nullptr)            { menu_type = EditType::Widget; }
    else if (a_selectedGameObject != nullptr)   { menu_type = EditType::GameObject; }
    else if (a_selectedLight != nullptr)	    { menu_type = EditType::Light; }
    ...
    if (menu_type == EditType::GameObject)
    {
